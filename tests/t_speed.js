...
ok(GameData.setSpeed(0.3) === 0.5 && GameData.setSpeed(10.7) === 10 && GameData.setSpeed(2.26) === 2.5 && GameData.setSpeed(2.24) === 2 && GameData.setSpeed('abc') === 1 && GameData.setSpeed(0) === 0.5, 'クランプと 0.5きざみ(0 → 0.5)');
...
ok(GameData.speedLabel() === '3.0×' && GameData.speedLabel(0.5) === '0.5×', 'ラベル');
GameData.setSpeed(1);
...
ok(H.drawn.some(s => s === '⏩ 2.0×'), 'プレイ中 左上に ⏩ 2.0×');
...
const r10 = playAt(10, g); ok(r10.result && r10.result.miss === 0 && r10.result.whiff === 0 && r10.result.rank === 'superb', '10×: ジャストなら ぜんぶ とれる', JSON.stringify(r10.result));
{
...
console.log('--- 3) ストップ中に はやさを かえる → いまの拍から 計算しなおし(メニューの ボタンは セレクトとは べつの id) ---');
{
  ...
  byId('btn-pspd-down').reset(); byId('btn-pspd-up').reset();   // メニューの ボタンは まいかい つくりなおされる
  key('Escape'); key('Escape', false);
  const pauseAt = clock.t, beatCur = (pauseAt - beat0) / spb;
  ok(ov.innerHTML.includes('id="btn-pspd-up"') && ov.innerHTML.includes('1.0×') && !ov.innerHTML.includes('id="btn-spd-up"'), 'ストップメニューに はやさ(セレクトの id とは かぶらない)');
  byId('btn-pspd-up').fire('click'); byId('btn-pspd-up').fire('click');
  ok(GameData.speed() === 2 && byId('pspd-now').textContent === '2.0×', '＋2回で 2.0×', GameData.speed() + ' ' + byId('pspd-now').textContent);
...
}
{
  ...
  byId('btn-pspd-down').reset(); byId('btn-pspd-up').reset();
  key('Escape'); key('Escape', false);
  const pauseAt = clock.t, beatCur = (pauseAt - beat0) / spb;
  byId('btn-pspd-down').fire('click');
  ok(GameData.speed() === 0.5 && byId('pspd-now').textContent === '0.5×', '−で 0.5×');
  byId('btn-pspd-down').fire('click');
  ok(GameData.speed() === 0.5, '0.5× より したには いかない');
  byId('btn-resume').fire('click');
  const resumeAt = clock.t + 1.5; let dt = null;
  while (dt == null) { clock.t += 0.002; frame(); if (clock.t >= resumeAt) dt = clock.t - pauseAt; }
  const tOf2 = b => pauseAt + dt + (b - beatCur) * spb * 2;
...
console.log('--- 4) セレクトの ボタン・スライダー・イントロ表示 ---');
{
  ...
  ok(byId('spd-label').textContent === '⏩ はやさ 1.0×' && byId('speed-ctl').hidden === false, 'ラベル 1.0×');
  byId('btn-spd-up').fire('click');
  ok(GameData.speed() === 1.5 && byId('spd-label').textContent === '⏩ はやさ 1.5×' && byId('spd-range').value === '1.5', '＋ → 1.5×');
  byId('spd-range').value = '4'; byId('spd-range').fire('input');
  ok(GameData.speed() === 4 && byId('spd-label').textContent === '⏩ はやさ 4.0×', 'スライダー → 4.0×');
  byId('btn-spd-down').fire('click');
  ok(GameData.speed() === 3.5, '− → 3.5×');
  GameData.setSpeed(10); byId('btn-spd-up').fire('click'); ok(GameData.speed() === 10, '10× より うえには いかない');
  GameData.setSpeed(0.5); byId('btn-spd-down').fire('click'); ok(GameData.speed() === 0.5, '0.5× より したには いかない');
  GameData.setSpeed(2);
  const btn = { dataset: { s: '1', slot: '0' }, classList: { add() {}, remove() {} } };
  byId('stage-list').fire('click', { target: { closest: () => btn } });
  ok(ov.innerHTML.includes('⏩ はやさ 2.0×'), 'イントロに はやさ');
  Engine.stop();
